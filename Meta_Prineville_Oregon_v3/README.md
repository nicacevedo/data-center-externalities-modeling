...
Working environment used here: conda env `dc_externalities`, with `PYTHONNOUSERSITE=1`. A local venv from `requirements.txt` is also fine. Raw files are expected to already exist for `python run_prineville.py full` (that command does not download MADIS).
...
- `data/source_manifest.csv`: originally catalogued public URLs, role, resolution, quality and whether manual action is needed. GWIS, FERC, CAMPD, EIA-860/923/cooling, DEQ, and Crook County permits are also used; the full executable inventory is in `docs/PIPELINE_DATA_MODEL_REPORT.md` and `docs/pipeline_report.pdf`.
...
The freeze was built in conda env `dc_externalities` with `export PYTHONNOUSERSITE=1`. After install, `python -m pytest` from this directory is the test check.
