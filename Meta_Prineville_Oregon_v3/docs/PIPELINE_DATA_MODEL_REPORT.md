...
- Source count: **50**. Quantity count: **82** (29 unavailable; 44 rows with implemented/partial implementation text). Boundary IDs in use: **15** (14 among non-unavailable quantities). Sensitivity rows: **5**.
...
- **Accounting boundary:** `NOT_IDENTIFIED` — Campus consumption is not identified. City SWR METER volume is not total return flow.
- **Validation?** none
- **Confidence:** none. Site consumption is not publicly reported as a separate campus series. City SWR METER is not identified as total return flow.
...
- **Provenance:** `reported` (implemented)
- **Accounting boundary:** `META_CAMPUS` — Meta-disclosed annual campus withdrawal. Not City customer-service meters, not City production, and not Vitesse/Facebook POD use.
- **Validation?** source status in meta_prineville_annual.csv; city_meta_annual_reconciliation.csv is boundary diagnostic only
- **Confidence:** high as disclosed annual withdrawal. Monthly Meta campus-total withdrawal is still not observed. City utility meters observe a customer-service component, not this annual campus total. 2011-2013 missing.
### City-metered Facebook Data Center utility/service water (`W^{city,service}`)

- **What is it?** Sum of City Facebook Data Center meters whose rate code begins WATER - COMM or ADD'L WATER, in observed months. City-metered utility/service water, not total Meta withdrawal.
- **Where does it come from?** CITY_FB_METER_CONSUMPTION
- **How is it computed?** usage_gallons = usage_100cf × 748; usage_m3 = usage_gallons × 0.003785411784; sum included meters
- **Assumptions?** parallel WATER-COMM / ADD'L WATER meters are not demonstrated parent/submeters; aggregate is provisional
- **Provenance:** `reported` (implemented)
- **Accounting boundary:** `CITY_UTILITY_CUSTOMER_DELIVERY` — City-metered Facebook Data Center WATER-COMM + ADD'L WATER. Downstream customer-service boundary, not Meta campus total withdrawal.
- **Validation?** city_meter_qa.csv; city_meta_annual_reconciliation.csv
- **Confidence:** high as City-reported meter usage; provisional as a campus-service boundary. Does not include bulk, sewer, WELL METER FOR SEW, Trailer City, or Warehouse. Not groundwater.
...
Common-support window 2014-01–2024-12 (n=120). Lowest MAE: **seasonal_persistence** (MAE 7473 m³/month).


| model | n | MAE | RMSE | sMAPE | median AE |
|---|---:|---:|---:|---:|---:|
| seasonal_persistence | 120 | 7473 | 11165 | 0.631 | 4672 |
| scale_plus_evap | 120 | 8518 | 12686 | 0.612 | 4810 |
| annual_electricity_scale | 120 | 8693 | 12973 | 0.648 | 6237 |
| climatology | 120 | 9430 | 14692 | 0.862 | 5174 |
| graybox_evap_scale | 120 | 11728 | 15879 | 1.557 | 8461 |

Years beating seasonal persistence on common support: climatology 3/10; annual_electricity_scale 5/10; graybox_evap_scale 0/10; scale_plus_evap 5/10.

Native-support pooled scores remain in [`city_metered_service_model_scores.csv`](../outputs/city_prineville/city_metered_service_model_scores.csv) (secondary; n differs by model: [120, 132]).

Information sets (models are not rewritten to become forecastable):


| model | evaluation_role | forecastable_ex_ante | target-month weather | target-year electricity |
|---|---|---|---|---|
| climatology | prospective_forecastable_baseline | True | False | False |
| seasonal_persistence | prospective_forecastable_baseline | True | False | False |
| annual_electricity_scale | contemporaneous_explanatory | False | False | True |
| graybox_evap_scale | contemporaneous_explanatory | False | True | True |
| scale_plus_evap | contemporaneous_explanatory | False | True | True |

City-service share of Meta annual withdrawal is **not historically stable** (range 13–107% where both exist). service + bulk (bulk allocated by observed bill year) is an accounting-date diagnostic, not a water-balance closure. Recent close agreement in selected years does not establish a universal accounting identity or a fixed conversion/source-share.

Gray-box monthly shares show meaningful seasonality but over-concentrate use in summer. A purely evaporation-driven level model is insufficient. Possible interpretation, not a fitted decomposition: monthly City-service demand appears to contain a persistent facility-state / base component plus a weather-sensitive seasonal component. That base component is **not** identified as domestic, process, or non-cooling water.

Within-year share comparison: [`outputs/city_prineville/city_metered_service_graybox_shape.csv`](../outputs/city_prineville/city_metered_service_graybox_shape.csv), [shape figure](../outputs/city_prineville/figures/city_service_vs_graybox_shape.png). Components: [components](../outputs/city_prineville/figures/city_water_components_monthly.png). Annual boundary reconciliation: [Meta vs City](../outputs/city_prineville/figures/meta_city_annual_reconciliation.png).
...
- **Direct water consumption** (`Q_W_CONS`): Site consumption is not publicly reported as a separate campus series. City SWR METER is not identified as total return flow.
