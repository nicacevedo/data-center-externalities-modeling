...
This is interactive and should be exported manually because the result depends on entity/POD/right selection. Follow `MANUAL_ACQUISITION.md` section 1 exactly.

Important source-quality fields:
- measured vs estimated, if provided;
- water year vs calendar year;
- entity/right/POD identity;
- units and reporting completeness.

Normalize into `data/manual_templates/owrd_monthly_well_use.csv`.

Do not treat absence of a report as zero use: OWRD states that only a subset of rights are required to report.
