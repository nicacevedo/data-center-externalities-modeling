...
def build_system(
    ...
    if system_seed_mode == "orthogonal_v3":
        # Gamma is excluded so storage draws are identical across NONE/LOW/MED/HIGH.
        struct_rng = rng_for(structural_seed("system", topology, memory, recharge_efficiency))
    elif system_seed_mode == "legacy_v2":
        struct_rng = rng_for(structural_seed("system", topology, memory, gamma_label, recharge_efficiency))
    else:
        raise ValueError(f"unknown system_seed_mode {system_seed_mode!r}")
...
def generate_forcing(
    ...
    R_base = float(rspec["mean"]) * (1.0 + float(rspec["seasonal_amplitude"]) * s_R) + _ar1(
        streams["recharge_innovations"] if streams is not None else rng,
        float(rspec["ar1_phi"]),
        float(rspec["ar1_innovation_sd"]),
        length,
        n,
    )
    ...
    if streams is None:
        ...
        common_phi = 0.9
        log_sd = (
            latent_common_climate_sd / float(rspec["mean"])
            if latent_common_climate_sd > 0.0
            else 0.0
        )
        innovation_sd = log_sd * np.sqrt(1.0 - common_phi**2)
        common = _ar1(streams["latent_climate_innovations"], common_phi, max(innovation_sd, 0.0), length, 1)
        if latent_common_climate_sd > 0.0:
            R = R_base * np.exp(common - 0.5 * log_sd**2)
        else:
            R = R_base
        Q = (
            float(qspec["mean"]) * (1.0 + float(qspec["seasonal_amplitude"]) * s_Q)
            + _ar1(
                streams["pumping_innovations"],
                float(qspec["ar1_phi"]),
                float(qspec["ar1_innovation_sd"]),
                length,
                n,
            )
            + streams["pumping_excitation"].normal(
                0.0, float(qspec["independent_excitation_sd"]), size=(length, n)
            )
        )
...
def simulate(
    ...
    if streams is None:
        ...
        streams["initial_condition"].normal(0.0, 1.0, size=system.n_nodes)
        streams["recharge_initial"].normal(0.0, 1.0, size=system.n_nodes)
        streams["pumping_initial"].normal(0.0, 1.0, size=system.n_nodes)
        streams["latent_climate_initial"].normal(0.0, 1.0, size=1)
        independent = streams["placebo_independent"].normal(0.0, 1.0, size=R.shape)
        if placebo_corr > 0.0:
            r_z = (R - R.mean(axis=0)) / np.maximum(R.std(axis=0), 1e-12)
            mixed = placebo_corr * r_z + np.sqrt(max(1.0 - placebo_corr**2, 0.0)) * independent
            qspec = design["forcing"]["pumping"]
            Q_placebo = np.maximum(float(qspec["mean"]) * (1.0 + 0.4 * mixed), 0.0)
        z_eps = streams["process_noise"].normal(0.0, 1.0, size=(total, system.n_nodes))
        eps = z_eps * float(regime.process_noise_sd)
