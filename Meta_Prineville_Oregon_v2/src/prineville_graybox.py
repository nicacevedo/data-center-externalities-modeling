...
def simulate(weather: pd.DataFrame, p_it_mw, params=Params()):
    ...
    for i in range(len(w)):
        if not (np.isfinite(tdb[i]) and np.isfinite(t_supply[i]) and np.isfinite(rh[i]) and np.isfinite(p[i])):
            water_kg_s[i]=np.nan; continue
    ...
    p_evap_aux=0.005*pit*np.nan_to_num(spray,nan=0.0)
